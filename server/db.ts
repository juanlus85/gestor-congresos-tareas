...
export async function createMember(values: typeof members.$inferInsert) { const db = await getDb(); if (!db) throw new Error("La base de datos no está disponible"); const result = await db.insert(members).values(values); return Number(result[0].insertId); }
...
export async function replaceMemberCommittees(memberId: number, groupIds: number[]) { const db = await getDb(); if (!db) throw new Error("La base de datos no está disponible"); await db.delete(groupMembers).where(eq(groupMembers.memberId, memberId)); if (groupIds.length) await db.insert(groupMembers).values(groupIds.map(groupId => ({ groupId, memberId }))); }
